...
class TokenEncoder {
  ...
  encode(tokens, settings) {
    return new EncodeContext(this, tokens, settings);
  }

  getCode(token) {
    if (!isTokenTypeCustomized(token.type)) {
      var diff_codes = this.token_classes[token.type];
      if (diff_codes && diff_codes[token]) {
        return diff_codes[token];
      }
      if (!diff_codes) {
        this.token_classes[token.type] = diff_codes = {};
      }
      this.code = this.code + 1;
      diff_codes[token] = this.code;
      return this.code;
    } else {
      var token_codes = this.token_classes[token.type];
      if (!token_codes) {
        this.token_classes[token.type] = token_codes = [];
      }
      for (const token_pair of token_codes) {
        if (token_pair.token.equals(token)) {
          return token_pair.code;
        }
      }
      this.code = this.code + 1;
      token_codes.push({ token, code: this.code });
      return this.code;
    }
  }
}
...
class EncodeContext {
  constructor(encoder, text, options) {
    let re;
    if (text) {
      if (options.compare === 'chars' || options.compare === 'tokens') {
        // split all chars
        re = '';
...
class Myers {
  ...
  static diff(lhs, rhs, options = {}) {
    ...
    const encoder = (options.compare === 'tokens') ? new TokenEncoder() : new Encoder();
    ...
    const changes = [];
    const compare = (options.compare === 'chars' || options.compare === 'tokens') ? 0 : 1;
