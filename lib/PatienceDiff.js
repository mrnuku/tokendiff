...
function patienceDiff(aLines, bLines, accessorCb, equalityCb, diffPlusFlag) {
  ...
  function findUnique(arr, lo, hi) {
    ...
    lineMap.forEach((val, key, map) => {
      if (val.count !== 1) map.delete(key);
      else map.set(key, val.index);
    });
    return lineMap;
  }
  ...
  function uniqueCommon(aArray, aLo, aHi, bArray, bLo, bHi) {
    ...
    ma.forEach((val, key, map) => {
      if (mb.has(key) ) map.set(key, {indexA: val, indexB: mb.get( key )});
      else map.delete(key);
    });
    return ma;
  }
  ...
  const aMove = [], aMoveIndex = [], bMove = [], bMoveIndex = [];
  ...
  function addSubMatch(aLo, aHi, bLo, bHi) {
    return new Promise((resolve, reject) => {
      ...
      while (aLo <= aHi && bLo <= bHi && equalityCb(aLines[aLo], bLines[bLo])) addToResult(aLo++, bLo++);
      // Match any lines at the end of aLines and bLines, but don't place them
      // in the "result" array just yet, as the lines between these matches at
      // the beginning and the end need to be analyzed first.
      var aHiTemp = aHi;
      while ( aLo <= aHi && bLo <= bHi && equalityCb(aLines[aHi], bLines[bHi]) ) {
        aHi --;
        bHi --;
      }
      ...
      if ( uniqueCommonMap.size === 0 ) {
        ...
        setTimeout(async () => {
          await recurseLCS( aLo, aHi, bLo, bHi, uniqueCommonMap );
          // Finally, let's add the matches at the end to the result.
          while ( aHi < aHiTemp ) addToResult( ++ aHi, ++ bHi );
          resolve();
        }, 0);
  ...
  function recurseLCS(aLo, aHi, bLo, bHi, uniqueCommonMap) {
    return new Promise(async (resolve, reject) => {
      ...
      if (x.length === 0) {
        await addSubMatch(aLo, aHi, bLo, bHi);
      } else {
        if (aLo < x[0].indexA || bLo < x[0].indexB) await addSubMatch(aLo, x[0].indexA - 1, bLo, x[0].indexB - 1);
        var i;
        for (i = 0; i < x.length - 1; i++) await addSubMatch(x[i].indexA, x[i + 1].indexA - 1, x[i].indexB, x[i + 1].indexB - 1);
        if (x[i].indexA <= aHi || x[i].indexB <= bHi) await addSubMatch(x[i].indexA, aHi, x[i].indexB, bHi);
      }
      resolve();
    });
  }
  ...
  return new Promise(async (resolve, reject) => {
    await recurseLCS(0, aLines.length - 1, 0, bLines.length - 1);
...
function patienceDiffPlus(aLines, bLines, accessorCb, equalityCb) {
	return new Promise(async (resolve, reject) => {
	  const difference = await patienceDiff(aLines, bLines, accessorCb, equalityCb, true);
	  var aMoveNext = difference.aMove, aMoveIndexNext = difference.aMoveIndex, bMoveNext = difference.bMove, bMoveIndexNext = difference.bMoveIndex;
	  delete difference.aMove;
	  delete difference.aMoveIndex;
	  delete difference.bMove;
	  delete difference.bMoveIndex;
	  var lastLineCountMoved;
	  do {
	    var aMove = aMoveNext, aMoveIndex = aMoveIndexNext, bMove = bMoveNext, bMoveIndex = bMoveIndexNext;
	    aMoveNext = [];
	    aMoveIndexNext = [];
	    bMoveNext = [];
	    bMoveIndexNext = [];
	    var subDiff = await patienceDiff( aMove, bMove, accessorCb, equalityCb );
