...
function nuLexerLogLine(line, text) {
  this.line = line;
  this.text = text;
}
...
function nuLexer(sourceLines, fileName, flags, userPuncTable) {
  ...
  this.userPuncTable = _sortPuncTable(userPuncTable === undefined ? PTB : userPuncTable.slice().concat(PTB));
  this.errors = [];
  this.warnings = [];
}
...
nuLexer.prototype._error = function(text) {
  this.errors.push(new nuLexerLogLine(this.line, text));
  if (!this.flags.includes(LFL.NOERRORS)) process.stderr.write(`${this.fileName}:${this.line + 1} ERR ${text}\n`);
}

nuLexer.prototype._warning = function(text) {
  this.warnings.push(new nuLexerLogLine(this.line, text));
  if (!this.flags.includes(LFL.NOWARNINGS)) process.stderr.write(`${this.fileName}:${this.line + 1} WARN ${text}\n`);
}
  ...
  while (1) {
    ...
    if (this.source[0] == '/') { // skip comments
      if (this.source[1] == '/') { // comments //
        if (!this.sourceLines.length) { // comment is on the last line without newline
          if (emitWhiteSpaceData) {
            whiteSpaceStr += this.source;
            whitespace.str = whiteSpaceStr;
          }
          return 2;
        }
...
nuLexer.prototype.ReadString = function(token, quote) {
  ...
  while (1) {
    ...
    if (this.source[0] == '\\' && this.flags.includes(LFL.STRINGESCAPECHARS)) {
      ...
      str += ch.ch;
    } else if ( this.source[0] == '\\' && !this.flags.includes(LFL.STRINGESCAPECHARS) &&
                this.source.length > 1 && (this.source[1] == quote || this.source[1] == '\\')) {
      str += this.source[0];
      str += this.source[1];
      this._advanceColumn(2);
    } else if ( this.source[0] == '\\' && this.source.length == 1) {
      if (!this.sourceLines.length) {
        this._error("malformed multiline string");
        return 0;
      }
      this._advanceLine();
...
nuLexer.prototype.ReadNumber = function(token) {
  ...
  if (c == '0' && c2 != '.') {
    if (c2 == 'x' || c2 == 'X') { // check for a hexadecimal number
      ...
      if (str.length > 1)
        ...
        token.subtype = [TNT.DECIMAL, TNT.INTEGER];
    }
  ...
  if (token.subtype.includes(TNT.FLOAT)) {
    ...
    if (token.subtype.includes(TNT.DECIMAL) || token.subtype.includes(TNT.HEX))
      token.value = parseInt(str);
    else if (token.subtype.includes(TNT.BINARY))
      token.value = parseInt(str.slice(2), 2);
    else if (token.subtype.includes(TNT.OCTAL))
      token.value = parseInt(str.slice(1), 8);
...
nuLexer.CompareParsed = function(tokensA, tokensB) {
