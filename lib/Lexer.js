...
function nuLexer(sourceLines, flags, userPuncTable) {
...
nuLexer.prototype._error = function(text) {
  if (!this.flags.includes(LFL.NOERRORS)) console.warn(text);
}

nuLexer.prototype._warning = function(text) {
  if (!this.flags.includes(LFL.NOWARNINGS)) console.warn(text);
}

nuLexer.prototype.ReadWhiteSpace = function() {
  while (1) {
...
nuLexer.prototype.ReadToken = function(token) {
  if (!this.ReadWhiteSpace()) return 0; // read white space before token
  if (this.flags.includes(LFL.POSITIONS)) {
  ...
  if (this.flags.includes(LFL.ONLYSTRINGS)) { // if we're keeping everything as whitespace deliminated strings
    ...
    if (this.flags.includes(LFL.NOUNKNOWN)) {
...
nuLexer.prototype.Parse = function() {
  ...
  while (1) {
    const token = {};
    if (this.ReadToken(token)) out.push(token);
    else break;
  }
  return out;
}
...
module.exports = { LFL, TT, TNT, PT, PTB, nuLexer };
